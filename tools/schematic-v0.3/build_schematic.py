...
OUT = BASE.parents[1] / 'hardware' / '原理图_v0.3_封装与接口完善'
OUT.mkdir(parents=True, exist_ok=True)
REF = zipfile.ZipFile(BASE.parent / 'schematic-v0.1/references/SF58-official.epro')
ORIG = json.loads(REF.read('project.json'))
TITLE = 'HealthWatch-SF58-BareChip-v0.3'
def uid(key): return uuid.uuid5(uuid.NAMESPACE_URL, 'healthwatch-sf58-v03/'+key).hex
...
def symbol(key, units, width=240, passive=None):
    ...
    for part, pins in units:
        ...
        for num,name,typ,side in pins:
            ...
            rows.append(['ATTR','g'+str(seq),pid,'Pin Type',{'Input':'IN','Output':'OUT','Bidirectional':'BI'}.get(typ,typ),0,0,x,y,0,'hidden',0]);seq+=1
            locations[num]={'x':x,'y':y,'side':side,'name':name,'type':typ,'id':pid}
...
class Sheet:
    def __init__(self,title,subtitle):
        ...
        self.text(65,75,'REV 0.3 | 2026-09-11 | SCHEMATIC DRAFT / NOT FOR FABRICATION','small')
        self.text(1540,75,'Named nets connect across sheets. Cross = EDA no-connect flag.','small')
        SHEETS.append(self)
    ...
    def place(self,ref,did,part,x,y,nets,value=None,note=''):
        ...
        for num,p in sym['pins'].items():
            ...
            if n is None:
                self.attr(c+'-'+p['id'],'NO_CONNECT','yes',px,py)
...
s.note(120,450,[
 ...
 'BGA footprint is bound; stackup, fanout, crystal layout and assembly process still require PCB review.',
...
j=ic('DEBUG_1x07',[(str(i),n,'Passive','L') for i,n in enumerate(['VTREF_1V8','GND','SWDIO','SWCLK','RESET_N','MCU_TX','MCU_RX'],1)],'','Defined 1.27mm pitch pogo / wire pads',width=240)
...
flash=ic('PY25Q128HA-WXH-IR',[pin(8,'VCC','L','Power'),pin(1,'CS_N'),pin(6,'CLK'),pin(5,'D0','L','Bidirectional'),pin(2,'D1','R','Bidirectional'),pin(3,'D2_WP_N','R','Bidirectional'),pin(7,'D3_HOLD_N','R','Bidirectional'),pin(4,'GND','R','Ground'),pin(9,'EP','R','Ground')],
 'https://www.puyasemi.com/en/h_series653/3187.html','WSON8+EP 6x5mm, WX suffix verified against Puya V2.3',width=230)
...
usb_names={1:'A1B12_GND',2:'A4B9_VBUS',3:'A8_SBU1',4:'A5_CC1',5:'B7_DM2',6:'A6_DP1',7:'A7_DM1',8:'B6_DP2',9:'B8_SBU2',10:'B5_CC2',11:'B4A9_VBUS',12:'B1A12_GND',13:'SHELL1',14:'SHELL2',15:'SHELL3',16:'SHELL4',17:'MOUNT_NC1',18:'MOUNT_NC2'}
...
two(s,'F201','F','1206L050/15YR 0.5A 15V','USB_VBUS_RAW','USB_5V',320,850,pkg='Fuse footprint TBD',note='0.5A hold is at room temperature. Derate input current at elevated ambient; validate fuse self-heating.')
...
bat=ic('BATTERY_1x03',[pin(1,'PACK+','L','Power'),pin(2,'PACK-','L','Ground'),pin(3,'NTC10k','L','Passive')],'','Defined 3-pad wire interface: +, -, NTC; verify pack polarity',width=230)
...
two(s,'L301','L','2.2uH XFL3012-222MEC','BUCK18_SW','+1V8',1020,1330)
...
two(s,'L302','L','1.5uH XFL3012-152MEC','BB33_L1','BB33_L2',1020,900)
...
two(s,'L303','L','2.2uH XFL3012-222MEC','VSYS','BOOST5_SW',1020,450)
...
s.note(1170,720,['Purchased 1.85in QSPI AMOLED kit confirmed. Use its adapter; check orientation before powering.',
...
crown=ic('CROWN_AB_SWITCH',[pin(1,'ENC_A','L','Passive'),pin(2,'ENC_COMMON','L','Passive'),pin(3,'ENC_B','L','Passive'),pin(4,'PUSH','L','Passive'),pin(5,'PUSH_COMMON','L','Passive')],'','Defined 5-pad wire interface; final rotary encoder mechanics pending',width=230)
...
lra=ic('LRA_2PIN',[pin(1,'LRA+','L','Passive'),pin(2,'LRA-','L','Passive')],'','Defined 2-pad wire interface; LRA voltage/resonance pending',width=200)
...
two(s,'Y601','Y','32.768kHz CL6pF CM1610H32768DZB','PPG_XI','PPG_XO',1430,480,pkg='Crystal 2pin / footprint TBD',note='ESR <90k, C0<2pF; confirm load network against selected crystal and reference design')
...
green=ic('CT DBLP31.12-6C5D-56-J6U6',[pin('1','ANODE','L','Passive'),pin('2','CATHODE','R','Passive'),pin('3','CATHODE_THERMAL','R','Passive')],'https://look.ams-osram.com/m/7a84e13abf7bd695/original/CT-DBLP31-12.pdf','OSRAM 2218, 3 physical pads; ADI reference land pattern',width=230)
s.place('D702',green,'A',1550,1080,{'1':'+5V_SW','2':'GREEN1_K','3':'GREEN1_K'})
s.place('D703',green,'A',1550,840,{'1':'+5V_SW','2':'GREEN2_K','3':'GREEN2_K'})
...
s.note(160,470,['Current is regulated by MAXM86146 LED drivers. Set pulse current / timing before acquisition.',
 ...
 'Green LED: physical pad 1 = anode; pads 2 and 3 = cathode. Both cathode pads are wired.',
...
s.note(140,420,[
 ...
 'EDA no-connect flags mark deliberate unused pins. Configure unused digital inputs to avoid floating input current.',
...
s.note(130,470,[
 ...
 'BGA pad geometry is taken from the official chip footprint. PCB placement/routing and fabrication release remain pending.'
])
...
with (OUT/'BOM_v0.3.csv').open('w',encoding='utf-8-sig',newline='') as f:
...
with (OUT/'pin-net-map_v0.3.csv').open('w',encoding='utf-8-sig',newline='') as f:
