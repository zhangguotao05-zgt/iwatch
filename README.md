...
D13-D 的代码修正提交为 `50dee36`：长按进入选择器，支持 DIGITAL/MODULAR_LOCAL 两个稳定表盘 ID、颜色/中心/左右快捷入口草稿、取消、revision 冲突和能力失败回滚；表盘候选只有在根页异步 `ONRESUME` 成功且稳定快照确认后才提交，请求拒绝、异步失败、故障回收和重复返回均回滚。主机产品矩阵已补齐上述路由时序；GCC/Keil 已重新构建并生成绑定源码 HEAD、工具链和 SDK 的身份归档，GCC/Keil `main.bin` 分别为 1,951,900 B/1,877,096 B。开发板刷写、表盘实屏回归和 D13 总体收口仍待架构师复核后执行。完整 Python 门禁仍受本机 clean-base SDK 污染影响；#8 性能、#10 显示异常、D15 持久化和 Issue #15 继续保持打开。详见 [D13-D 记录](docs/ui/D13-D_表盘会话与总体收口实施记录.html)。
