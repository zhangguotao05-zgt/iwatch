...
- [Apple Watch 功能差距与复刻验收矩阵（当前差距、视觉问题与 D14–D18 清单）](docs/ui/Apple_Watch功能差距与复刻验收矩阵_v1.html)
...
D13-D 的代码修正提交为 `a2e61fd`：长按进入选择器，支持 DIGITAL/MODULAR_LOCAL 两个稳定表盘 ID、颜色/中心/左右快捷入口草稿、取消、revision 冲突和能力失败回滚；提交成功后同步根页模型，Home 与 Apply 同周期时撤销 pending，tick 从 0 开始也能触发 3 秒超时回滚。表盘候选只有在根页异步 `ONRESUME` 成功且稳定快照确认后才提交，请求拒绝、异步失败、故障回收和重复返回均回滚。主机产品矩阵、46 项隔离 clean-base Python 门禁已通过；GCC/Keil 已重新构建并生成绑定源码 HEAD、工具链和 SDK 的身份归档，GCC/Keil `main.bin` 分别为 1,952,020 B/1,877,184 B。与 `e324f14` 归档绑定的 GCC 三件套已经由 COM9 写入并校验，COM10 原始日志 `work/d13d-board-e324/serial.raw` 为 28,798 字节，SHA-256 为 `d9d2854aa2c0c0c49d02c86ec2c536194fb0019b8b0038cd65306e40db8303de`；正确的 `iwface/root` 表盘编辑、根页异步提交确认、取消和重复进出均已实机记录。`2892732` 是实机证据基线，`8587d13` 是文档内容基线，`b7d51aa` 是状态提交；D13-D 已获架构复核通过，`main` 已推送；Issue #15 暂不关闭。#8 性能、#10 显示异常和 D15 持久化继续保持打开。详见 [D13-D 记录](docs/ui/D13-D_表盘会话与总体收口实施记录.html)。

Apple Watch 功能范围与复刻差距已单独整理为 [功能差距与复刻验收矩阵](docs/ui/Apple_Watch功能差距与复刻验收矩阵_v1.html)。工程师按矩阵中的 A/B/C/D/E 状态、D14–D18 顺序和“主机／目标构建／实机”三层证据推进；知乎文章仅作功能索引，不把规划清单或示意图当作已实现页面。联网、支付、健康传感器算法、触感／音频和量产硬件能力仍不属于当前离线 UI 交付。
