...
D07 v2 已以 `967483d` 为批准基线。D07-A1 已获架构审批通过，收口提交为 e064686。D07-B 已获架构阶段验收通过（审批基线 7dfd90d），开始 D07-C 五类共享组件及多使用方故障清理。首次按键延迟超目标和触摸缓冲覆盖继续跟踪；D07-D 最终验收未完成，Issue #7 保持打开。 字体保持 336 字符、56,804 B。新镜像 20%/80% 下文字与操作正常，8 次进出后的主堆和字形池均回到启动值。详见 [安全整改记录](docs/ui/D07-A1_安全整改与复验记录.html)。`flash1` 的 FAL 持久化、低功耗和长期测试仍未关闭。
