...
D07 v2 已以 `967483d` 为批准基线。D07-A1 已获架构审批通过，收口提交为 e064686。D07-B 字体服务与主题已完成主机、双工具链和正常实机复验，按键延迟波动单列跟踪；C/D 尚未开始，D07整体未收口。 字体保持 336 字符、56,804 B。新镜像 20%/80% 下文字与操作正常，8 次进出后的主堆和字形池均回到启动值。详见 [安全整改记录](docs/ui/D07-A1_安全整改与复验记录.html)。`flash1` 的 FAL 持久化、低功耗和长期测试仍未关闭。
