...
D07 v2 已以 `967483d` 为批准基线。A1 的 336 字符子集为 56,804 B，GCC main 降至约 1.86 MB（6 MiB 槽的 29.625%）。字距缓存重复释放及 EPIC 字形缓存清理长度遗漏均已修复；主机 4,046 次像素提交对照、双工具链和 GCC 实机回归通过。用户确认文字及 20%/80% 亮度正常，6 次进出后主堆和字形池回到初始值，36 个输入队列样本的 P95/最大延迟均为 13 ms。**A1 工程验证完成，待架构复核；B/C 新组件未开始。** 详见 [A1 记录](docs/ui/D07-A1_字体替换与渲染修复记录.html)。`flash1` 的 FAL 持久化、低功耗和长期测试尚未关闭。仓库目前没有可直接投产的佩戴版 PCB 或完整 watchOS 界面。
