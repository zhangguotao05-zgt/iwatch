...
D01–D03 已完成页面生命周期、BSP/分区/双工具链来源、GUI 等待/唤醒和输入取消实现，并在开发板上完成备份、镜像回读、冷启动与输入压力验收。DEV_A128_NAND 的 main 位于 `0x69000000`，必须与同一归档的 Bootloader、FTab 成套烧录。当前下一批为 D04/D05 的时间、启动协调和最小服务基础。
