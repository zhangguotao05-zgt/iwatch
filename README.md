...
D07 v2 已以 `967483d` 为批准基线，当前正在 A1 字体替换与实屏整改。336 字符子集为 56,804 B，GCC main 已降至约 1.86 MB（6 MiB 槽的 29.625%）。首轮上板发现字距缓存重复释放及中文破损：前者已关闭缓存并复验进入页面，后者已定位到 EPIC 连续绘制遗漏字形缓存清理长度，主机 4,046 次像素提交对照通过，修复包仍待实屏复验。**A1 未验收，B/C 新组件未开始。** `flash1` 的 FAL 持久化、低功耗和长期测试尚未关闭。仓库目前没有可直接投产的佩戴版 PCB 或完整 watchOS 界面。
