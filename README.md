...
D12 已实现 16 个会话内闹钟、24 槽提醒账本和全局视觉提醒页；[实施与验收记录](docs/ui/D12_闹钟与全局提醒实施记录.html)分别归档主机、双工具链及开发板结果。实机验证了真实到期、容量边界、隐藏重进及闹钟创建／编辑／删除；一次自动恢复的 LCD 超时继续由 #10 跟踪。开发板不报告扬声器或马达能力，声音／振动不能按实机通过填写；Issue #14 在架构复核前保持打开。
