...
当前 main 已占 6 MiB 槽约 91%。D07 v2 已以 `967483d` 为批准基线；A0 的确定性字体子集、双工具链预算解析、受控 SDK 补丁、主机 OOM 注入和替换前实屏基线均已完成。基线照片确认旧完整 TTF 的长中文正文存在字形重叠、局部断裂和行间覆盖，亮度、输入和生命周期状态正常；该缺陷是 A1 必须关闭的对照项。`flash1` 的 FAL 持久化、低功耗和长期测试尚未关闭。仓库目前没有可直接投产的佩戴版 PCB 或完整 watchOS 界面。
