...
D07 v2 已以 `967483d` 为批准基线。D07-A1 三项安全整改已通过主机回归、双工具链构建和新 GCC 镜像实机正常路径复验，待架构复核；A1 尚未正式收口，B/C 未开始。 字体保持 336 字符、56,804 B。新镜像 20%/80% 下文字与操作正常，8 次进出后的主堆和字形池均回到启动值。详见 [安全整改记录](docs/ui/D07-A1_安全整改与复验记录.html)。`flash1` 的 FAL 持久化、低功耗和长期测试仍未关闭。
