...
- [D13 系统覆盖层与表盘管理设计 v2](docs/ui/D13_系统覆盖层与表盘管理设计_v2.html) · [390×450 页面视觉目标](docs/ui/D13_系统页面视觉目标_v1.html) · [D13-C 锁与输入仲裁实施记录](docs/ui/D13-C_锁与输入仲裁实施记录.html)
...
D13-A 以 `c7617c2` 获架构批准，D13-B 增量已完成控制中心、通知、叠放和切换器实现。D13-C 锁与输入仲裁增量以源码 `49d461f` 完成架构复核和开发板验收：实体 KEY2 可打开控制中心，普通锁／水锁均支持 KEY1 长按约 2 秒并真实松手解锁，锁页和控制中心正确销毁并返回根页；水锁下的触摸与 KEY2 屏蔽已通过板上诊断注入验证。实体触摸面板本轮未单独确认，不能把诊断注入写成实体触摸通过。GCC/Keil 身份均绑定 `49d461f`，COM9/COM10 原始证据已封存；完整 Python 门禁仍受本机 clean-base SDK 环境污染影响。详细边界见 [D13-C 记录](docs/ui/D13-C_锁与输入仲裁实施记录.html)。表盘编辑与持久化不在本批范围。Issue #15、#8、#10 与 D15 继续保持打开。
