...
#define V00_DISPLAY_TRACK_FIRST 121
#define V00_DISPLAY_TRACK_LAST 269
...
uint8_t iw_brightness_v00_display_level(int32_t x) {
    if (x <= V00_DISPLAY_TRACK_FIRST) return IW_BRIGHTNESS_MIN;
    if (x >= V00_DISPLAY_TRACK_LAST) return IW_BRIGHTNESS_MAX;
    unsigned span = V00_DISPLAY_TRACK_LAST - V00_DISPLAY_TRACK_FIRST;
    return (uint8_t)(IW_BRIGHTNESS_MIN +
                     ((unsigned)(x - V00_DISPLAY_TRACK_FIRST) *
                      (IW_BRIGHTNESS_MAX - IW_BRIGHTNESS_MIN) + span / 2) / span);
}
