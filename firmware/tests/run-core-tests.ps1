...
try {
    ...
    & cl.exe /nologo /std:c11 /utf-8 /W4 /WX /Od /Z7 "/I$coreDir" /Fetest_alerts.exe (Join-Path $PSScriptRoot 'test_alerts.c') (Join-Path $coreDir 'iw_alerts.c')
    if ($LASTEXITCODE -ne 0) { throw 'D12 关键提醒账本测试编译失败。' }
    & ./test_alerts.exe
    if ($LASTEXITCODE -ne 0) { throw 'D12 T18/T19 关键提醒账本测试失败。' }
    & cl.exe /nologo /std:c11 /utf-8 /W4 /WX /Od /Z7 "/I$coreDir" /Fetest_alert_output.exe (Join-Path $PSScriptRoot 'test_alert_output.c') (Join-Path $coreDir 'iw_alert_output.c') (Join-Path $coreDir 'iw_alerts.c')
    if ($LASTEXITCODE -ne 0) { throw 'D12 输出管理测试编译失败。' }
    & ./test_alert_output.exe
    if ($LASTEXITCODE -ne 0) { throw 'D12 输出管理测试失败。' }
    & cl.exe /nologo /std:c11 /utf-8 /W4 /WX /Od /Z7 "/I$coreDir" /Fetest_alarms.exe (Join-Path $PSScriptRoot 'test_alarms.c') (Join-Path $coreDir 'iw_alarms.c') (Join-Path $coreDir 'iw_alerts.c') (Join-Path $coreDir 'iw_time.c')
    ...
    & cl.exe /nologo /std:c11 /utf-8 /W4 /WX /Od /Z7 "/I$coreDir" "/I$serviceDir" /Fetest_service.exe (Join-Path $PSScriptRoot 'test_service.c') (Join-Path $serviceDir 'iw_service.c') (Join-Path $coreDir 'iw_time.c') (Join-Path $coreDir 'iw_chronograph.c') (Join-Path $coreDir 'iw_alerts.c') (Join-Path $coreDir 'iw_alarms.c') (Join-Path $coreDir 'iw_alert_output.c')
    ...
    & cl.exe /nologo /std:c11 /utf-8 /W4 /WX /Od /Z7 "/I$guiCoreDir" "/I$coreDir" "/I$serviceDir" /Fetest_ui_commands.exe (Join-Path $PSScriptRoot 'test_ui_commands.c') (Join-Path $guiCoreDir 'iw_ui_commands.c') (Join-Path $serviceDir 'iw_service.c') (Join-Path $coreDir 'iw_time.c') (Join-Path $coreDir 'iw_chronograph.c') (Join-Path $coreDir 'iw_alerts.c') (Join-Path $coreDir 'iw_alarms.c') (Join-Path $coreDir 'iw_alert_output.c')
